...
.DESCRIPTION
    ...
    Account, by attempting to import & publish any and all runbooks in successive waves to avoid cumbersome
    source folder structures, especially since this really only matters for the first publish of a runbook.
...
.NOTES
    ...
    LASTEDIT: 
#>
workflow Sync-VsoGitRmRunbookWF 
{
    [CmdletBinding()]	
    param (
    ...
    foreach ($item in $results.value)
    {
        ...
        elseif (($item.gitObjectType -eq "blob") -and ($item.path -match $psExtension -or $item.path -match $grExtension))
        {
            ...
            $fileName = InlineScript { $path = $Using:item.path; [AzureAutomationVsoGitSync.Models.Runbook]::GetRunbookFileName($path); }
            ...
            InlineScript {
                $allRbs = $Using:allRunbooks
                $path = $Using:outFile
                $url = $Using:fileUrl
                $file = $Using:fileName
                $new = $allRbs.Add($path, $url)
            }
        }
    }
    ...
    if ($FlatFilesMode)
    {
        ...
        { 
            ...
            # (Runbooks should be in bottom-up order)
